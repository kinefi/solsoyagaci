import { Scale, BookOpen } from 'lucide-react'; // BookOpen ikonunu ekliyoruz
    ...
    <div className="absolute top-5 left-5 z-10 bg-slate-800/90 backdrop-blur p-5 rounded-2xl border border-slate-700 shadow-xl max-w-md">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">Tarihsel Soyağacı ve İttifaklar Ağı</p>
        </div>

        <div className="flex bg-slate-900/80 p-1 rounded-xl border border-slate-700">
          <button
            onClick={() => setViewMode('map')}
            className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition cursor-pointer ${viewMode === 'map' ? 'bg-slate-700 text-white shadow' : 'text-slate-400 hover:text-slate-200'
              }`}
          >
            Harita
          </button>
          <button
            ...
            className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition cursor-pointer ${viewMode === 'table' ? 'bg-slate-700 text-white shadow' : 'text-slate-400 hover:text-slate-200'
              }`}
          >
            Tablo
          </button>
      ...
      <a
        href="https://fliphtml5.com/peavd/xeud/H%C3%BCseyin_Aykol_-_T%C3%BCrkiye%26%2339%3Bde_Sol_%C3%96rg%C3%BCtler%2C_B%C3%B6l%C3%BCne_B%C3%B6l%C3%BCne_B%C3%BCy%C3%BCmek_(Phoenix_Yay%C4%B1nevi%2C_2022)/262"
        target="_blank"
        rel="noopener noreferrer"
        className="mt-3 pt-3 border-t border-slate-700/60 flex items-start gap-2 text-slate-300 bg-slate-900/40 p-2.5 rounded-xl border border-slate-800 hover:border-slate-600 transition group"
      >
        <BookOpen size={16} className="text-red-400 shrink-0 mt-0.5 group-hover:text-red-300 transition" />
        <div className="text-[11px] leading-relaxed">
          <span className="font-semibold text-slate-200 group-hover:text-white transition">Kaynak Eser:</span> Hüseyin Aykol - <span className="italic text-slate-300">Türkiye'de Sol Örgütler / Bölüne Bölüne Büyümek</span> kitabının <span className="text-red-300 font-medium">sf. 259-264</span> "Türkiye Solunun Dört Damarı" bölümündeki 4 listeden yararlanılmıştır.
        </div>
      </a>
